...
{
    double[] coord = FindCoords(lineDatal, lineData2); Console.Write($"Точа пересечения уравнений y={lineDatal[COEFFICIENT]}*x+{lineDatal[CONSTANT]} и y={lineData2[COEFFICIENT]}*x+{lineData2[CONSTANT]} ");
    Console.WriteLine($" имеет координаты ({coord[X_COORD]}, {coord[Y_COORD]})");
} // Ввод числа double Prompt(string message) {
...
if (lineDatal[CONSTANT] == lineData2[CONSTANT])
    ...
    {
        Console.WriteLine("Прямые параллельны"); return false;
    }
) 
return true;
}
